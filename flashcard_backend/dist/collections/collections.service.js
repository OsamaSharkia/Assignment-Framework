"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CollectionsService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const collection_entity_1 = require("./entities/collection.entity");
const flashcard_set_entity_1 = require("../flashcards/entities/flashcard-set.entity");
const comment_entity_1 = require("../flashcards/entities/comment.entity");
let CollectionsService = class CollectionsService {
    constructor(collectionRepository, flashcardSetRepository, commentRepository) {
        this.collectionRepository = collectionRepository;
        this.flashcardSetRepository = flashcardSetRepository;
        this.commentRepository = commentRepository;
    }
    async findAllCollections() {
        return this.collectionRepository.find({ relations: ['sets'] });
    }
    async redirectToRandomCollection() {
        const collections = await this.collectionRepository.find();
        if (!collections.length) {
            throw new common_1.NotFoundException('No collections found');
        }
        const randomIndex = Math.floor(Math.random() * collections.length);
        return collections[randomIndex];
    }
    async findCollectionById(id) {
        return this.collectionRepository.findOne({
            where: { id },
            relations: ['sets', 'comments'],
        });
    }
    async createCollection(collectionDto) {
        const flashcardSets = await this.flashcardSetRepository.findByIds(collectionDto.sets);
        const collection = this.collectionRepository.create({ name: collectionDto.name, sets: flashcardSets });
        return this.collectionRepository.save(collection);
    }
    async updateCollectionById(id, collectionDto) {
        const collection = await this.findCollectionById(id);
        if (collectionDto.sets && collectionDto.sets.length) {
            const flashcardSets = await this.flashcardSetRepository.findByIds(collectionDto.sets);
            collection.sets = flashcardSets;
        }
        collection.name = collectionDto.name || collection.name;
        return this.collectionRepository.save(collection);
    }
    async addCommentToCollection(collectionId, commentDto) {
        const collection = await this.collectionRepository.findOne({
            where: { id: collectionId },
            relations: ['comments'],
        });
        if (!collection) {
            throw new Error('Collection not found');
        }
        const comment = this.commentRepository.create({
            content: commentDto.content,
            collection,
        });
        return await this.commentRepository.save(comment);
    }
    async deleteCollectionById(id) {
        const result = await this.collectionRepository.delete(id);
        if (result.affected === 0) {
            throw new common_1.NotFoundException(`Collection with ID ${id} not found`);
        }
    }
};
exports.CollectionsService = CollectionsService;
exports.CollectionsService = CollectionsService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(collection_entity_1.Collection)),
    __param(1, (0, typeorm_1.InjectRepository)(flashcard_set_entity_1.FlashcardSet)),
    __param(2, (0, typeorm_1.InjectRepository)(comment_entity_1.Comment)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.Repository])
], CollectionsService);
//# sourceMappingURL=collections.service.js.map